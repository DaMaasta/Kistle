...
const BOX_COLORS = ["#f97316","#ef4444","#eab308","#22c55e","#14b8a6","#3b82f6","#8b5cf6","#ec4899"];
...
export default function GroupDetail({ navigate, params }: GroupDetailProps): React.ReactElement {
  ...
  const closeReturn = () => { setReturnBooking(null); setReturnConfirm(false); setReturnError(null); setReturnSuccess(false); };
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setNewName(""); setNewDesc(""); setNewColor("#f97316"); setShowCreate(false);
  ...
  return (
    <div style={styles.container}>
      ...
      {/* Zurückbuchen Modal */}
      {returnBooking && (
        <div style={styles.modalOverlay} onClick={closeReturn}>
          <div style={styles.returnModal} onClick={(e) => e.stopPropagation()}>
            {!returnConfirm ? (
              <>
                ...
                <div style={styles.returnActions}>
                  ...
                  <button
                    style={{
                      ...
                      background: returnSuccess ? "linear-gradient(135deg,#16a34a,#15803d)" : "linear-gradient(135deg,#f97316,#ea580c)",
      ...
      {/* Box löschen Modal */}
      {deleteBox && (
        <div style={styles.modalOverlay} onClick={() => setDeleteBox(null)}>
          <div style={styles.qrModal} onClick={(e) => e.stopPropagation()}>
            <div style={styles.qrModalHeader}>
            ...
            <p style={{ fontSize: 14, color: "var(--c-text-2)", margin: "8px 0 20px", lineHeight: 1.5 }}>
          ...
          {showCreate && (
            <div style={styles.createCard}>
              ...
              <div style={styles.createActions}>
                <button style={styles.cancelBtn} onClick={() => { setShowCreate(false); setNewName(""); setNewDesc(""); setNewColor("#f97316"); }}>Abbrechen</button>
              ...
              {boxes.map((box) => (
                <React.Fragment key={box.id}>
                  {editingId === box.id ? (
                    ...
                    <div style={{ ...styles.boxCard, boxShadow: `inset 0 0 0 1px ${box.color ?? "#f97316"}55, var(--neu-raised-sm)` }}>
                      <button style={styles.boxClickable}
                        onClick={() => navigate("BoxDetail", { box, place: group })}>
                        <div style={styles.boxTop}>
                          <Package size={22} color={box.color ?? "#f97316"} />
                                      ...
                                      >
                                        <div style={styles.bookingItemLeft}>
                                          ...
                                          <span style={styles.bookingItemLocation}>
                                            <MapPin size={11} color="#f97316" style={{ flexShrink: 0 }} />
...
const styles: Record<string, CSSProperties> = {
  ...
  qrCopyBtn: { flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, background: "linear-gradient(135deg, #f97316, #ea580c)", color: "#fff", border: "none", borderRadius: 10, padding: "10px 0", fontSize: 13, fontWeight: 700, cursor: "pointer" },
  ...
  personLabelAvatar: { width: 24, height: 24, borderRadius: "50%", background: "linear-gradient(135deg, #f97316, #ea580c)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, flexShrink: 0 },
  ...
  bookingItemLocation: { display: "flex", alignItems: "center", gap: 3, fontSize: 11, color: "#f97316", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const },
  bookingItemQty: { fontSize: 13, fontWeight: 700, color: "#f97316", flexShrink: 0 },
  ...
  youBadge: { fontSize: 10, fontWeight: 700, color: "#f97316", background: "var(--c-accent-bg)", borderRadius: 6, padding: "1px 6px" },
  ...
  inviteConfirmBtn: { flex: 1, background: "linear-gradient(135deg, #f97316, #ea580c)", border: "none", borderRadius: 10, padding: "11px 0", fontSize: 13, fontWeight: 700, color: "#fff", cursor: "pointer" },
  returnBtnInline: { background: "#f97316", border: "none", borderRadius: 11, cursor: "pointer", width: 44, height: 44, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, margin: "0 8px" },
  ...
  returnConfirmBtn: { flex: 1, background: "linear-gradient(135deg, #f97316, #ea580c)", border: "none", borderRadius: 12, padding: "13px 0", fontSize: 14, fontWeight: 700, color: "#fff", cursor: "pointer" },
};
